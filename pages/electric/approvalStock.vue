<template>
	<view class="main  main-top-bg2 bg-my-gray" >
		<cu-custom class="head" :isBack="true">
			...
			<block slot="content">出库审批</block>
		</cu-custom>
		<scroll-view scroll-y class="detail-content padding-sm  bg-my-gray margin-top-sm" >
			...
			<view v-for="(item,index) in goodsList" :key="index">
				<view class="cu-card case no-card margin-top-sm">
					<view class="cu-item padding-xs border-left-color">
						...
						<view class="flex detail">
							<view class="flex-sub ">备注：</view>
							<view class="flex-sub  text-left">{{item.REMARK}}</view>
						</view>
					</view>
				</view>
			</view>
			<view class="bg-white">
				<view class="flex detail margin-top padding-top-sm">
					...
					<radio-group class="margin-right" @change="radioChange">
						<view class="flex-sub margin-right">
							<radio class='green margin-left margin-right-sm' :class="isTrue=='1'?'checked':''" :checked="isTrue=='1'?true:false"
							 value="1"></radio>同意
							<radio class='red margin-left margin-right-sm' :class="isTrue=='0'?'checked':''" :checked="isTrue=='0'?true:false"
							 value="0"></radio>不同意
						</view>
					</radio-group>
				</view>
	...
	export default {
		data() {
			return {
				...
				divisionName: ['上次审核', '部门负责人审批', '上次审核', '安生部负责人审批'],
