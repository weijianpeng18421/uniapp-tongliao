	...
	<view class="main  main-top-bg2 bg-my-gray" >
		...
		<scroll-view scroll-y class="detail-content  bg-my-gray margin-top-sm " >
			<view class="cu-card case no-card border-left-color">
				<view class="cu-item padding-xs">
					...
					<view class="flex detail ">
						...
						<view class="flex-sub text-sm">{{formList.APPLY_PERSON_NAME}}</view>
					</view>
					<view class="flex detail">
					...
					<view class="flex detail">
						<view class="flex-sub ">计划金额合计：</view>
						<view class="flex-sub  text-left">￥{{formList.PURCHASE_MONEY}}</view>
					</view>
					<view class="flex detail">
			...
			<view class="bg-white ">
				...
				<view class="cu-form-group margin-top-sm" style="border: 1px solid #dbd5e2;">
	...
	export default {
		...
		methods: {
			...
			saveData(){
				...
				data.apply_status_name = _this.name
			...
			goTimeLine() {
				var _this = this
				uni.navigateTo({
					url: 'timeline?type=apply'+'&uid='+ _this.uid +'&unit=' + _this.formList.DETP_NAME + '&name=' + _this.formList.APPLY_PERSON_NAME + '&time=' + _this.applyTime
				});
			},
			//模态框
			showModal() {
